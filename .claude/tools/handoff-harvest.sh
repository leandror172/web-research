...
# 3. Find the newest commit with subject starting with 'chore(session-handoff): session '
...
# 5. If not found:
#    - Print "handoff-harvest: no 'chore(session-handoff): session ' commit found; falling back to last 20 commits" to stderr
...
# Find the newest commit with subject starting with 'chore(session-handoff): session '
HANDOFF_SHA=$(git log -n1 --format=%H --grep='^chore(session-handoff): session ' || true)

if [[ -z "$HANDOFF_SHA" ]]; then
  # No handoff commit found — fall back to last 20 commits
  echo "handoff-harvest: no 'chore(session-handoff): session ' commit found; falling back to last 20 commits" >&2
